...
function Face({ thinking, still }: { thinking: boolean; still: boolean }) {
  return (
    <motion.span
      ...
      className="relative block aspect-[144/167] w-10"
      aria-hidden
      initial={false}
      ...
      */}
      <div className="relative mt-3 min-h-[min(15rem,40svh)] grow [@media(max-height:520px)]:min-h-[6.5rem]">
        <div
          ...
          className="no-scrollbar absolute inset-0 overflow-y-auto overscroll-contain pl-12 [mask-image:linear-gradient(to_bottom,transparent_0,black_1.5rem)]"
        >
          {empty ? (
